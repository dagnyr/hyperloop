...
# TO DO:
# Filter outputs to have one per residue and show min and max distance between atoms.
# Only include TCR - other chain pairs.
# IGMT align sequences and extract CDRs / other regions.
# Map residues from interaction table to regions.
...
function check_atom_chemistry!(db, pdb_id, atomA, atomB, resA, resB, idA, classA, idB, classB)
    ...
    if dist > 5.0 # using cutoff from gliph paper
        return
    end
...
function check_pi_stacking!(db, pdb_id, resA, resB, idA, classA, idB, classB)
    ...
    angle = get_inter_ring_angle(normA, normB) # angle has to be <30 or between 60 and 90
...
function get_annotations(pdb_id::String)
    ...
    end # end of chain mapping check
...
function get_interactions!(db, pdb_id, struc, annotations)
    ...
    for (i, chainA) in pairs(chains)
        ...
        if roleA === nothing || occursin("Solvent", roleA) # check if atom is solvent
            continue
        end
        ...
        end # end of comparing to other chains

    end # main chain loop end
...
pdb_ids = ["1AO7", "2BAN"] # just for testing
