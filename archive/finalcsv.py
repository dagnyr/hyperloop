...
SALTBRIDGE_PATH = "out/tcr_pmhc_saltbridges_all.tsv"               # your tcr-peptide salt bridge output
OUT_PATH        = "out/saltbridges_with_metadata_tpep.csv"  # output
# ----------------------------------------------
...
def main():
    ...
    required_sb_cols = ["pdb_id", "tcr_chain", "tcr_resid", "peptide_chain", "peptide_resid", "min_distance"]
    ...
    sb_df["peptide_chain"] = sb_df["peptide_chain"].astype(str).str.strip()

    # Parse residue numbers for easier downstream work
    sb_df["tcr_resseq"] = sb_df["tcr_resid"].apply(parse_resseq)
    sb_df["peptide_resseq"] = sb_df["peptide_resid"].apply(parse_resseq)
    ...
    pep_map = chain_df.rename(columns={
        chain_key: "peptide_chain",
        "chain_asym": "peptide_chain_asym",
        "label": "peptide_chain_label",
        "description": "peptide_chain_description",
    })[["pdb_id", "peptide_chain", "peptide_chain_asym", "peptide_chain_label", "peptide_chain_description"]]

    merged = merged.merge(pep_map, on=["pdb_id", "peptide_chain"], how="left")
    ...
    first = [
        ...
        "peptide_chain", "peptide_chain_asym", "peptide_chain_label", "peptide_chain_description",
        "peptide_resid", "peptide_resseq",
        "min_distance",
    ]
    ...
    missing_pep = merged["peptide_chain_label"].isna().sum() if "peptide_chain_label" in merged.columns else 0
