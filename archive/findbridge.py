...
# ---- load chain mapping CSV into a dict: pdb_id -> {"tcr": [...], "pmhc": [...]} ----
chain_map = {}
...
for pdb_id in pdb_ids:
    ...
    try:
        ...
        if pdb_id in chain_map:
            ...
            pmhc_chain_ids = chain_map[pdb_id]["pmhc"]
        else:
            if skip_if_no_mapping:
                print(f"{pdb_id}: skipping (no chain mapping found in {mapping_csv})")
                continue
            tcr_chain_ids = default_tcr_chain_ids
            pmhc_chain_ids = default_pmhc_chain_ids

        if not tcr_chain_ids or not pmhc_chain_ids:
            print(f"{pdb_id}: skipping (empty chain mapping) tcr={tcr_chain_ids} pmhc={pmhc_chain_ids}")
            continue
        ...
        if not pmhc_chains_present:
            print(f"{pdb_id}: no mapped pMHC chains present. mapped={pmhc_chain_ids} available={sorted(available_chains)}")
            continue
